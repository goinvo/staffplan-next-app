...
export interface UserDataContextType {
	...
	setDeleteAssignment: React.Dispatch<
		React.SetStateAction<"archive" | "deleteMe" | "hide" | "">
	>;
	setNewProjectAssignmentId: React.Dispatch<
...
export const UserListProvider: React.FC<{
	...
	const [deleteAssignment, setDeleteAssignment] = useState<
		"archive" | "deleteMe" | "hide" | ""
	>("");
	const [newProjectAssignmentId, setNewProjectAssignmentId] = useState<
		number | null
	>(null);
	...
	const [sortOrder, setSortOrder] = useState<SORT_ORDER>(
		SORT_ORDER.ASC_COVERED
	);
	const [sortBy, setSortBy] = useState<string>("Client");
	const [viewsFilterPeople, setViewsFilterPeople] = useState("abcUserName");
	const [viewsFilterSingleUser, setViewsFilterSingleUser] =
		useState("byClient");
	const [assignmentsWithUndoActions, setAssignmentsWithUndoActions] = useState<
		UndoableModifiedAssignment[]
	>([]);
	...
	useEffect(() => {
		if (userListData) {
			const sortedUserList = sortUserListByOrder(
				sortOrder,
	...
	const setSelectedUserData = useCallback(
		(newSelectedId: number) => {
			...
			const selectedUser = userList.find(
				(user) => user.id?.toString() === newSelectedId.toString()
			);
			if (!selectedUser) return;

			const newAssignment = newProjectAssignmentId
				? selectedUser.assignments.filter(
						(a) => Number(a.project.id) === newProjectAssignmentId
				  )
				: [];
			...
			const filteredAssignments = assignmentsToSort.filter((a) => {
				if (
					showArchivedAssignments &&
					viewer?.id.toString() === newSelectedId.toString()
				) {
			...
			const singleUser = sortSingleUserByOrder(sortOrder, sortBy, {
				...selectedUser,
				assignments: filteredAssignments,
			});
			const singleUserToSet = {
				...singleUser,
				assignments: [...newAssignment, ...singleUser.assignments],
			};
	...
	const sortedAndFilteredUsers = useMemo(() => {
		...
		if (sortedList) {
			const filteredList = isFirstShowInactiveUsers
				? [
						...sortedList.filter((user) => user.isActive),
						...sortedList.filter((user) => !user.isActive),
				  ]
				: sortedList;
	...
	const handleFinalDelete = useCallback((assignment: AssignmentType) => {
		setAssignmentsWithUndoActions((prev) =>
			prev.filter(({ assignment: a }) => a.id !== assignment.id)
		);
	}, []);

	const undoModifyAssignment = useCallback(
		async (assignmentId: number) => {
			const restored = assignmentsWithUndoActions.find(
	...
	const enqueueTimer = ({
		...
		const focusedAssignments = userList.map((user) => {
			if (user.id === updatedAssignment.assignedUser.id) {
				...
				return { ...user, assignments: updatedAssignments };
			}
			return user;
		});
		const timerId = enqueueTask(async () => {
			try {
				await finalAction();
				await handleFinalDelete(updatedAssignment);
				setUserList(focusedAssignments);
			} catch (error) {
