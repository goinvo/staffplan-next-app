	...
	return (
		...
		>
			...
			{!showOneClientProjects && (
				<span
					className="block w-auto sm:max-w-[100px] md:max-w-[125px] lg:max-w-[140px] sm:w-full sm:mr-0 mr-2 cursor-pointer"
					onClick={() => {
						if (showOneClientProjects === project.client.id.toString()) {
							setShowOneClientProjects("");
						} else {
			...
			<div
				className={`flex w-auto sm:max-w-[170px] md:max-w-[220px] sm:w-full sm:ml-2 lg:ml-0 sm:mr-0 mr-2 sm:mt-0 mt-2 ${
					showOneClientProjects
						? "sm:!max-w-max md:!max-w-max lg:!max-w-max !ml-0 sm:mr-3"
						: ""
				}`}
			>
				...
				<div className={`sm:flex hidden justify-end w-auto ml-2 -mt-[5px]`}>
					<EllipsisProjectMenu
