...
export const UserLabel = ({
	...
	const updateUserList = (updatedProject: ProjectType) => {
		setUserList((prev) =>
			prev.map((u) => {
				if (
					u.id === viewer?.id &&
					updatedProject?.assignments?.some(
						(a: AssignmentType) => a.assignedUser.id === viewer?.id
					)
				) {
			...
			return prevUserList?.map((user) => {
				if (user.id === upsertAssignment.assignedUser.id) {
					const updatedUserAssignments = user.assignments.map((assignment) =>
						assignment.id === upsertAssignment.id
							? upsertAssignment
							: assignment
					);
					return {
	...
	const undoArchivedStatus = useCallback(
		async (assignmentsWithUndoActions: UndoableModifiedAssignment[]) => {
			...
			const assignmentBeforeModified = assignmentsWithUndoActions.find(
				(el: UndoableModifiedAssignment) => el.assignment.id === assignment.id
			);
			const variables = {
				id: assignment.id,
				projectId: projectId,
	...
	const undoArchivedProjectStatus = useCallback(
		async (projectsWithUndoActions: UndoableModifiedProject[]) => {
			const projectBeforeModified = projectsWithUndoActions.find(
				(p: UndoableModifiedProject) => p.project.id === assignment.project.id
			);
			const input = {
	...
	const handleArchiveAssignmentClick = async () => {
		if (assignment.project && assignment.project.isTempProject) {
			const removedTempAssignment = selectedUser?.assignments.filter(
				(a: AssignmentType) => a.id !== assignment.id
			);
		...
		if (assignment.status !== "archived") {
			...
			try {
				...
				if (response && response.data) {
					...
					const undoAction = (
						undoableAssignments: UndoableModifiedAssignment[]
					) => {
						undoArchivedStatus(undoableAssignments);
					};
					enqueueTimer({
						assignment,
						updatedAssignment,
		...
		try {
			...
			if (response && response.data) {
				...
				const undoAction = (
					...
					undoArchivedProjectStatus(projectsWithUndoActions);
				};
				enqueueProjectTimer({
					project,
					updatedProject,
	...
	const handleHideProject = async (project: ProjectType) => {
		...
		try {
			...
			if (
				response &&
				response.data &&
				!response.data.upsertAssignment.focused
			) {
				const updatedAssignment = response.data.upsertAssignment;
				const undoAction = (
					undoableAssignments: UndoableModifiedAssignment[]
				) => {
					undoHideProject(undoableAssignments);
				};
				enqueueTimer({
					assignment,
					updatedAssignment,
	...
	const downloadCSV = () => {
		...
		link.setAttribute(
			"download",
			`${assignment.project.client.name}-${assignment.project.name}.csv`
		);
	...
	const assignmentDropMenuOptions = [
		...
		},
		{
			component: (
				...
				>
					Delete {assignment.assignedUser.name.split(" ")[0]} from this project
				</button>
			),
		...
		},
		{
			component: (
				<button
					...
					className="block w-full px-4 py-2 text-sm text-left border-t border-t-[#E5E7EB] cursor-pointer"
				>
		...
		<div
			className={`w-full ${
				isAssignmentProposed
					? "sm:max-w-[185px] w-full md:pl-1 lg:pl-2"
					: "sm:max-w-[205px] w-full md:pl-1 lg:pl-2"
			} sm:mr-0 mr-2 flex items-start ${isFirstClient ? "mb-4 sm:mb-0" : ""}`}
		>
			<div>
				<button
